...
from .custom_utils import custom_gather_info, custom_test_episode

log = logging.getLogger(__name__)


class CustomBaseTrainer(ABC):
    ...
    def __init__(
        ...
        episode_per_test_dict: int | None = None,
        ...
        self.test_collector_dict = test_collector_dict
        ...
        self.episode_per_test_dict = episode_per_test_dict
    ...
    def _reset_collectors(self, reset_buffer: bool = False) -> None:
        ...
        # custom add-on
        if self.test_collector_dict is not None and len(self.test_collector_dict)>0:
            for test_collector_name, test_collector in self.test_collector_dict.items():
                test_collector.reset(reset_buffer=reset_buffer)
    ...
    def reset(self, reset_collectors: bool = True, reset_buffer: bool = False) -> None:
        ...
        if self.train_collector is not None and (
            self.train_collector.policy != self.policy or self.test_collector_dict is None
        ):
            self.test_in_train = False

        # custom add-on
        if (self.test_collector_dict is not None) and len(self.test_collector_dict)>0:
            assert self.episode_per_test_dict is not None
            
            for test_collector_name, test_collector in self.test_collector_dict.items():

                assert not isinstance(test_collector, AsyncCollector)  # Issue 700
                n_episode = self.episode_per_test_dict[test_collector_name]
                test_result = custom_test_episode(
                    collector=test_collector,
                    test_fn=self.test_fn,
                    epoch=self.start_epoch,
                    n_episode=n_episode,
                    is_final_update=None,
                    logger=self.logger,
    ...
    def __next__(self) -> EpochStats:
        ...
        if not self.stop_fn_flag:
            ...
            # test
            if self.test_collector_dict is not None:
                test_stat, self.stop_fn_flag = self.test_step()

        # custom add-on
        info_stat = custom_gather_info(
            ...
            test_collector_dict=self.test_collector_dict,
        )
    ...
    def test_step(self) -> tuple[CollectStats, bool]:
        ...
        assert self.episode_per_test_dict is not None
        assert self.test_collector_dict is not None
        stop_fn_flag = False

        # custom add-on
        # for i in range(len(self.test_collector_dict)):
        test_stat = None
        for test_collector_name, test_collector in self.test_collector_dict.items():
            n_episode = self.episode_per_test_dict[test_collector_name]

            test_stat0 = custom_test_episode(
                collector=test_collector,
                test_fn=self.test_fn,
                epoch=self.epoch,
                n_episode=n_episode,
            ...
            if 'eval_plus' in test_collector_name:
                test_stat = test_stat0
    ...
    def training_step(self) -> tuple[CollectStatsBase, TrainingStats | None, bool]:
        ...
        with policy_within_training_step(self.policy):
            ...
            # custom add-on
            print(f'Training: {self.policy.training}')
...
class CustomOnpolicyTrainer(CustomBaseTrainer):
    ...
    # for mypy
    assert isinstance(CustomBaseTrainer.__doc__, str)
    __doc__ = CustomBaseTrainer.gen_doc("onpolicy") + "\n".join(CustomBaseTrainer.__doc__.split("\n")[1:])
