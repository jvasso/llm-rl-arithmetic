...
from .custom_a2c_policy import CustomA2CPolicy


class CustomPPOPolicy(CustomA2CPolicy):
    ...
    def __init__(
        ...
        actor_sft: torch.nn.Module,
        optim: torch.optim.Optimizer,
        dist_fn: Type[torch.distributions.Distribution],
        kl_div_coef: float = 0.1,
        ...
        **kwargs: Any,
        ...
        super().__init__(actor=actor, critic=critic, action_space=action_space,
                         actor_sft=actor_sft, optim=optim, dist_fn=dist_fn, kl_div_coef=kl_div_coef, certainty_scaling_exponent=certainty_scaling_exponent, **kwargs)
    ...
    def learn(  # type: ignore
        ...
        losses, clip_losses, vf_losses, ent_losses, kl_div_losses = [], [], [], [], []
        for step in range(repeat):
            ...
            for minibatch in batch.split(batch_size, merge_last=True):
                ...
                if self.kl_div_coef != 0:
                    ...
                    kl_div_loss = self.compute_kl_div_penalty(minibatch=minibatch, log_probs_actions=log_prob)
                    loss = clip_loss + self._weight_vf * vf_loss \
                        - self._weight_ent * ent_loss - self.kl_div_coef * kl_div_loss
                else:
                    kl_div_loss = None
                    loss = clip_loss + self._weight_vf * vf_loss \
                        - self._weight_ent * ent_loss
                ...
                if kl_div_loss is not None:
                    kl_div_losses.append(kl_div_loss.item())
                else:
                    kl_div_losses.append(None)
